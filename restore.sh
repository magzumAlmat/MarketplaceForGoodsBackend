...
docker exec "$CONTAINER_NAME" env PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -d postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$RESTORE_DB_NAME' AND pid <> pg_backend_pid();"

# Drop existing restore database if it exists
log "Dropping existing $RESTORE_DB_NAME if it exists..."
docker exec "$CONTAINER_NAME" env PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -d postgres -c "DROP DATABASE IF EXISTS $RESTORE_DB_NAME;"
if [ $? -ne 0 ]; then
...
docker exec "$CONTAINER_NAME" env PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -d postgres -c "CREATE DATABASE $RESTORE_DB_NAME;"
if [ $? -ne 0 ]; then
...
if [ $? -eq 0 ]; then
    ...
    if [ "$RESET_SEQUENCES" = "y" ]; then
        ...
        docker exec "$CONTAINER_NAME" env PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -d "$RESTORE_DB_NAME" -c "DO \$\$ DECLARE r record; BEGIN FOR r IN (SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = 'public') LOOP EXECUTE 'ALTER SEQUENCE ' || quote_ident(r.sequence_name) || ' RESTART WITH 1;'; END LOOP; END \$\$"
        if [ $? -eq 0 ]; then
...
echo "   docker exec $CONTAINER_NAME env PGPASSWORD=\"$DB_PASSWORD\" pg_dump -U $DB_USER $DB_NAME --verbose | gzip > \"$BACKUP_DIR/${DB_NAME}_backup_before_restore_\$DATE.sql.gz\""
echo "3. If satisfied, replace the original database:"
echo "   docker exec $CONTAINER_NAME env PGPASSWORD=\"$DB_PASSWORD\" psql -U $DB_USER -d postgres -c 'DROP DATABASE IF EXISTS $DB_NAME;'"
echo "   docker exec $CONTAINER_NAME env PGPASSWORD=\"$DB_PASSWORD\" psql -U $DB_USER -d postgres -c 'ALTER DATABASE $RESTORE_DB_NAME RENAME TO $DB_NAME;'"
